...
conditionStr, partStr = f.read().split("\n\n")
...
for line in partStr.split("\n"):
    partList.append(re.split(",", line[1:-1]))


def applyRuleToPart(rule, part):
    for condition in rule[0:-1]:
        ...
        for category in part:
            categoryName, categoryValue = category.split("=")
            if conditionName == categoryName:
                if operator == "<":
                    conditionFound = int(categoryValue) < int(conditionValue)
                elif operator == ">":
                    conditionFound = int(categoryValue) > int(conditionValue)
            if conditionFound:
                return transition
...
for part in partList:
    x = "in"
    while x not in ["A", "R"]:
        x = applyRuleToPart(conditionDict[x], part)
    if x == "A":
        for category in part:
            sum += int(category[2:])
