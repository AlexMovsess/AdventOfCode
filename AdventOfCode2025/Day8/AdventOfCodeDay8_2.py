...
list_distances_sorted = [
    entry[0] for entry in sorted(distances.items(), key=lambda item: item[1])
]
...
for i in range(len(list_distances_sorted)):
    ...
    if len(sorted(circuit_list, key=lambda item: len(item), reverse=True)[0]) == len(
        lines
    ):
        print(
            coordinates[list_distances_sorted[i][0]][0]
            * coordinates[list_distances_sorted[i][1]][0]
        )
        break
